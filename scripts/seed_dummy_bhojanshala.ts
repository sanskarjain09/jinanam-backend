import { PrismaClient, OrganizationType, MealType, EventStatus, RoleKey } from '@prisma/client';
...
async function main() {
  ...
  const seq = await prisma.iDSequence.upsert({
    ...
    create: { prefix: 'JFBJ', currentSequence: 100 },
  });
  await prisma.organization.update({
    where: { id: org.id },
    data: { publicId: `JFBJ${seq.currentSequence}` }
  });
  ...
  const evSeq = await prisma.iDSequence.upsert({
    where: { prefix: 'JFEV' },
    update: { currentSequence: { increment: 1 } },
    create: { prefix: 'JFEV', currentSequence: 100 },
  });
  
  await prisma.event.create({
    data: {
      publicId: `JFEV${evSeq.currentSequence}`,
      organizationId: org.id,
  ...
  const bpSeq = await prisma.iDSequence.upsert({
    where: { prefix: 'JFBP' },
    update: { currentSequence: { increment: 1 } },
    create: { prefix: 'JFBP', currentSequence: 100 },
  });
  await prisma.bhojanshalaPass.create({
    data: {
      publicId: `JFBP${bpSeq.currentSequence}`,
      organizationId: org.id,
      memberId: memberId,
      mealType: MealType.LUNCH_AND_DINNER,
